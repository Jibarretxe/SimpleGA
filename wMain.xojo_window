...
Begin Window wMain
   ...
   Begin TextField txtYRange
      ...
      Width           =   80
   End
   Begin TextField txtMutationRate
      AcceptTabs      =   False
      Alignment       =   0
      AutoDeactivate  =   True
      AutomaticallyCheckSpelling=   False
   ...
   Begin Label Label6
      ...
      Width           =   100
   End
   Begin TextField txtPopulationSize
      AcceptTabs      =   False
      Alignment       =   0
      AutoDeactivate  =   True
      AutomaticallyCheckSpelling=   False
      ...
      Width           =   80
   End
   Begin TextField txtNumberOfClones
      AcceptTabs      =   False
      Alignment       =   0
      AutoDeactivate  =   True
      AutomaticallyCheckSpelling=   False
      ...
      Width           =   80
   End
   Begin TextField txtNumberOfCrosses
      AcceptTabs      =   False
      Alignment       =   0
      AutoDeactivate  =   True
      AutomaticallyCheckSpelling=   False
   ...
   Begin Label Label7
      ...
      Width           =   120
   End
   Begin TextField txtNumberOfIterations
      AcceptTabs      =   False
      Alignment       =   0
      AutoDeactivate  =   True
      AutomaticallyCheckSpelling=   False
...
#tag WindowCode
	#tag Method, Flags = &h21
		Private Sub FillAllParamMessage()
		  msgbox ("First enter all parameters")
	...
	#tag Property, Flags = &h0
		CurrentPopulation() As clsIndividual
	#tag EndProperty

	#tag Property, Flags = &h0
		TempPopulation() As clsIndividual
	#tag EndProperty
...
#tag Events pbStart
	#tag Event
		Sub Action()
		  ...
		  if val(txtMutationRate.text)>0 then
		    MutationRate=val(txtMutationRate.text)
		  else
		    FillAllParamMessage
		    exit sub
		  end if
		  
		  if val(txtNumberOfClones.text)>0 then
		    NumberOfClones=val(txtNumberOfClones.text)
		  else
		    FillAllParamMessage
		    exit sub
		  end if
		  
		  if val(txtnumberOfCrosses.text)>0 then
		    NumberOfCrosses=val(txtnumberOfCrosses.text)
		  else
		    FillAllParamMessage
		    exit sub
		  end if
		  
		  if val(txtPopulationSize.text)>0 then
		    PopulationSize=val(txtPopulationSize.text)
		  else
		    FillAllParamMessage
		    exit sub
		  end if
		  
		  if val(txtYRange.text)>0 then
		    RangeY=val(txtYRange.text)
		  else
		    FillAllParamMessage
		    exit sub
		  end if
		  
		  if val(txtXRange.text)>0 then
		    RangeX=val(txtXRange.text)
		  else
		    FillAllParamMessage
		    exit sub
		  end if
		  ...
		  if val(txtEliteSize.text)>0 then
		    EliteSize=val(txtEliteSize.text)
		  else
		    FillAllParamMessage
		    exit sub
		  end if
		  ...
		  Dim SortingArray() as integer
		  
		  For i=1 to PopulationSize
		    Dim NewIndividual as new clsIndividual
		    CurrentPopulation.Append NewIndividual
		    SortingArray.append NewIndividual.value
		  next i
		  
		  'Sort according to the value for each individual
		  SortingArray.SortWith(CurrentPopulation)
		  
		  //////////////////////////////////////////
		  'Here the next generations will be created.
		  'The top individuals will be kept. Also they will be cloned and then mutated and crossed over.
		  
		  For i= 1 to NumberOfIterations
		    ReDim TempPopulation(-1)
		    
		    'Select the top elite for the next generation
		    for j=0 to EliteSize-1
		      TempPopulation.Append CurrentPopulation(j)
		    next j
		    
		    'Clone the elite and mutate them
		    for j=0 to NumberOfClones-1
		      'first clone
		      TempPopulation.Append CurrentPopulation(j).Clone
